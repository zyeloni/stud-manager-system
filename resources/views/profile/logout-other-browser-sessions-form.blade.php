<x-jet-action-section>
    ...
    <x-slot name="description">
        {{ __('Zarządaj i wyloguj inne zalogowane urządzenia.') }}
    </x-slot>

    <x-slot name="content">
        <div class="max-w-xl text-sm text-gray-600">
            {{ __('Wszystkie dostępne sesje logowań.') }}
        </div>
                        ...
                        <div class="ml-3">
                            ...
                            <div>
                                <div class="text-xs text-gray-500">
                                    ...
                                    @else
                                        {{ __('Ostatio aktywny:') }} {{ $session->last_active }}
                                    @endif
                                </div>
                            </div>
                        </div>
        ...
        <x-jet-dialog-modal wire:model="confirmingLogout">
            <x-slot name="title">
                {{ __('Wylogowanie innych urządzeń') }}
            </x-slot>

            <x-slot name="content">
                {{ __('Wprowadź hasło do swojego konta.') }}
